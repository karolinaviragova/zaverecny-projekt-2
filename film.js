const filmy = [
	...
	{
		id: 'vykoupeni-z-veznice-shawshank',
		nazev: 'Vykoupení z věznice Shawshank',
		plakat: {
			url: 'https://image.pmgstatic.com/cache/resized/w720/files/images/film/posters/162/505/162505167_735db9.jpg',
			sirka: 420,
			vyska: 592,
		},
		ochutnavka: 'Mladý bankéř Andy Dufresne (Tim Robbins) je v roce 1947 odsouzen za vraždu své ženy a jejího milence.',
		popis:
...
const promennaNazev = document.querySelector("#detail-filmu .card-title");
const promennaText = document.querySelector("#detail-filmu .card-text");
const promennaPlakat = document.querySelector("#detail-filmu img");
...
filmy.forEach((film)=>{
	if(film.id === filmID){
		...
		promennaNazev.textContent = `${film.nazev}`
		promennaText.textContent = `${film.popis}`
		promennaPlakat.src = `${film.plakat.url}`
		premiera.innerHTML = `Premiéra <strong>${datum}</strong>`
	}
})

const elementFormulare = document.querySelector("#note-form");
const messageInput = document.querySelector("#message-input");
const checkbox = document.querySelector("#terms-checkbox");

elementFormulare.addEventListener("submit", (event) => {
	event.preventDefault();
	if(messageInput.value !== ""){
		if(checkbox.checked){
			elementFormulare.innerHTML = `<p class="card-text">${messageInput.value}</p>`
		} else {
